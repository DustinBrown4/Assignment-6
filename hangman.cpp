...
void mainMenu() {
	...
	//limit guesses to number of characters + 3 
	int hintCount = 0;
	int wrongCount = 0;
	string userGuess;
	do {
		...
		cout << "\t\ttries left: " << (wordToGuess.size() - wrongCount) << endl;
		...
		if (userGuess.size() == 1) {
			...
			if (found == string::npos) {
				...
				wrongCount += 1;
				if (wrongCount == (wordToGuess.size()+3)){
					cout << "You've guessed incorrectly too many times, game over." << endl;
					wordFound = true;
				}
			}
